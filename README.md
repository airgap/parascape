...
**Next portable** (deps satisfied, fan-in desc): `button-dropdown` (needs icon+dropdown+box+button+tooltip+popover+container ✅), `plugins`, `annotation-context` (needs alert+box+button+popover+container+space-between ✅), `calendar` (needs tooltip+button+grid+header ✅), `mixed-line-bar-chart`, `area-chart`.
