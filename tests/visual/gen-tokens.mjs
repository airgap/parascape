...
function candidates(name) {
  ...
  if (name === "--color-border-container-default") extra.push("colorBorderContainerTop", "colorBorderDividerDefault");
  if (name === "--color-border-item-focused") extra.push("colorBorderItemFocused", "colorBorderItemFocusedDefault");
  if (name === "--color-background-button-primary-default") extra.push("colorBackgroundButtonPrimaryDefault");
  if (name === "--color-background-button-primary-hover") extra.push("colorBackgroundButtonPrimaryHover");
  return [c, ...extra];
}

const names = [...new Set([...css.matchAll(/(--[a-z0-9-]+):/g)].map(x => x[1]))];
const prior = Object.fromEntries([...css.matchAll(/(--[a-z0-9-]+):\s*([^;]+);/g)].map(x => [x[1], x[2].trim()]));
...
console.log(`tokens: ${names.length}, real-mapped: ${names.length - unmapped.length}, kept-approx: ${unmapped.length}`);
