...
for (const ds of Object.values(deps))
	for (const d of ds) fanIn[norm(d)] = (fanIn[norm(d)] || 0) + 1;
...
for (const r of rows)
	lines.push(
		`| \`${r.name}\` | \`@cloudscape-design/components/${r.id}\` | ${r.tier} | ${r.fanin} | ${r.deps} |`,
	);
lines.push('');
lines.push(
	'**Next portable** (deps satisfied, fan-in desc): ' +
		nextReady
			.map((n) => `\`${n.c}\`${n.deps.length ? ` (needs ${n.deps.join('+')} ✅)` : ''}`)
			.join(', ') +
		'.',
);
lines.push('');
