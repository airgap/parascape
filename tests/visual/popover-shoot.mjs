...
async function shootBody(browser, url, { clickTrigger }) {
  ...
  if (clickTrigger) {
    // The Cloudscape trigger resolves but never passes Playwright's
    // "visible/enabled/stable" actionability gate (the popover-fixture
    // container override leaves the page in a perpetual non-stable
    // layout). We only need the click HANDLER to fire to open the
    // popover, so dispatch the event directly — bypasses actionability.
    const trigger = page.locator('[class*="awsui_trigger"]').first();
