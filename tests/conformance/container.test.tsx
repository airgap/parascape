// AUTO-ADAPTED from cloudscape-design/components src/container/__tests__/
// container.test.tsx via tests/conformance/codemod.mjs.
...
test('renders media if it is provided', () => {
	...
	const wrapper = renderContainer(
		<Container media={{ content: <img src={url} /> }}>test content</Container>,
	);
	expect(wrapper.findHeader()).toBeNull();
	expect(wrapper.findContent().getElement()).toHaveTextContent('test content');
...
describe('Style API', () => {
	test('all style properties', () => {
		...
		expect(getComputedStyle(wrapper.getElement()).getPropertyValue('background')).toBe(
			'rgb(240, 240, 235)',
		);
		...
		expect(
			getComputedStyle(
				wrapper.findByClassName(styles['content-inner'])!.getElement(),
		...
		expect(
			getComputedStyle(wrapper.findHeader()!.getElement()).getPropertyValue('padding-block'),
		).toBe('20px 0px');
		expect(
			getComputedStyle(wrapper.findHeader()!.getElement()).getPropertyValue('padding-inline'),
		).toBe('140px');
		expect(
			getComputedStyle(wrapper.findFooter()!.getElement()).getPropertyValue('border-color'),
		).toBe('purple');
		expect(
			getComputedStyle(wrapper.findFooter()!.getElement()).getPropertyValue('border-width'),
		).toBe('6px');
		expect(
			getComputedStyle(wrapper.findFooter()!.getElement()).getPropertyValue('padding-block'),
		).toBe('40px');
		expect(
			getComputedStyle(wrapper.findFooter()!.getElement()).getPropertyValue('padding-inline'),
		).toBe('140px');
	});
	...
	test('media top', () => {
		...
		expect(
			getComputedStyle(wrapper.findMedia()!.getElement()).getPropertyValue('border-radius'),
		).toBe('20px');
		expect(
			getComputedStyle(wrapper.findMedia()!.getElement()).getPropertyValue(
				'border-end-start-radius',
			),
		).toBe('0px');
		expect(
			getComputedStyle(wrapper.findMedia()!.getElement()).getPropertyValue('border-end-end-radius'),
		).toBe('0px');
	});
