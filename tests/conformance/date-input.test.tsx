// AUTO-ADAPTED from cloudscape-design/components src/date-input/__tests__/
// date-input.test.tsx via tests/conformance/codemod.mjs.
...
import DateInput from '@components/DateInput.pui';
import { createWrapper } from '@conformance/adapter';

function renderDateInput(props: any) {
	...
	return {
		...
		inputValue,
		onChangeSpy,
		rerender: (newProps: any) => rerender(<DateInput onChange={onChangeSpy} {...newProps} />),
	};
}
...
describe('Date Input component', () => {
	...
	describe('autocomplete', () => {
		['-', '/', '.', ' '].forEach((separatorKey) => {
			test(`should autocomplete using [${separatorKey}] key`, () => {
				...
				expect(onChangeSpy).toHaveBeenCalledWith(
					expect.objectContaining({ detail: { value: '2001-' } }),
				);
			});
		});
		...
		test('should correct "2" to "2002/01/01" on enter', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2002-01-01' } }),
			);
		});
		...
		test('should correct "21" to "2021/01/01" on enter', () => {
			...
			wrapper.findNativeInput().keydown({ key: 'Enter', keyCode: KeyCode.enter });
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2021-01-01' } }),
			);
		});
	});
	...
	describe('limiting range', () => {
		...
		test('should correct "2018/1/" to "2018/01/"', () => {
			...
			wrapper.findNativeInput().keydown({ key: '/' });
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-01-' } }),
			);
		});
		...
		test('should allow entry of day 31 in January', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-01-31' } }),
			);
		});
		...
		test('should allow entry of day 30 in March', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-03-30' } }),
			);
		});
		...
		test('should disallow entry of day 31 in April', () => {
			...
			wrapper.findNativeInput().keydown({ key: '1' });
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-04-30' } }),
			);
		});
		...
		test('should disallow entry of day 29 in february', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-02-28' } }),
			);
		});
	...
	describe('appending separator', () => {
		test('should automatically append separator after 4 digits entered', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-' } }),
			);
		});
		...
		test('should automatically append colon after 6 digits entered', () => {
			...
			wrapper.findNativeInput().keydown({ key: '1' });
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-11-' } }),
			);
		});
	...
	describe('entering value in middle of input', () => {
		test('should allow a value to be changed', () => {
			...
			wrapper.findNativeInput().keydown({ key: '9' });
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2019-01-02' } }),
			);
		});
		...
		test('should autocorrect if value is over limit', () => {
			...
			expect(onChangeSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { value: '2018-12-02' } }),
			);
		});
	...
	test('uses setInputValue test utils method', () => {
		...
		expect(onChangeSpy).toHaveBeenCalledWith(
			expect.objectContaining({ detail: { value: '2019-01-31' } }),
		);
	});
	...
	test.each([
		...
		{ inputFormat: 'iso' },
		{ inputFormat: 'slashed' },
	] as const)('accepts value in slashed format, inputFormat=$inputFormat', ({ inputFormat }) => {
	...
	test.each([
		...
		{ inputFormat: 'iso' },
		{ inputFormat: 'slashed' },
	] as const)('accepts value in iso format, inputFormat=$inputFormat', ({ inputFormat }) => {
		expect(renderDateInput({ value: '2001-02-03', inputFormat }).inputValue).toBe('2001/02/03');
	});

	test('displays value in slashed format', () => {
		expect(renderDateInput({ value: '2001-02-03', format: 'slashed' }).inputValue).toBe(
			'2001/02/03',
		);
	});
	...
	test('displays value in long-localized format', () => {
		expect(
			renderDateInput({ value: '2001-02-03', format: 'long-localized', locale: 'en-GB' })
				.inputValue,
		).toBe('3 February 2001');
	});
	...
	test.each([{ inputFormat: 'iso' }, { inputFormat: 'slashed' }] as const)(
		...
		({ inputFormat }) => {
			...
			expect(wrapper.findNativeInput().getElement().value).toBe(
				inputFormat === 'iso' ? '2001-02-03' : '2001/02/03',
			);
	...
	test('focuses input programmatically', () => {
		function TestComponent() {
			const inputRef = useRef<any.Ref>(null);
			return (
				<div>
	...
	describe('native attributes', () => {
		it('adds native attributes', () => {
			...
			expect(wrapper.getElement().querySelectorAll('input[data-testid="my-test-id"]')).toHaveLength(
				1,
			);
		});
	});

	describe('partial value handling for iso month format', () => {
		test('displays complete month value in iso format', () => {
			expect(
				renderDateInput({ value: '2024-06', format: 'iso', granularity: 'month' }).inputValue,
			).toBe('2024-06');
		});

		test('accepts typing into empty input', () => {
			const { wrapper, onChangeSpy } = renderDateInput({
				value: '',
				format: 'iso',
				granularity: 'month',
		...
		test('displays partial year value as-is', () => {
			expect(renderDateInput({ value: '20', format: 'iso', granularity: 'month' }).inputValue).toBe(
				'20',
			);
		});

		test('displays year with trailing separator as-is', () => {
			expect(
				renderDateInput({ value: '2024-', format: 'iso', granularity: 'month' }).inputValue,
			).toBe('2024-');
		});

		test('displays partial month value as-is', () => {
			expect(
				renderDateInput({ value: '2024-0', format: 'iso', granularity: 'month' }).inputValue,
			).toBe('2024-0');
		});
	});
});
