...
// render → adapter, styles → vendored, unported dep autosuggest; unported dep button-dropdown; unported dep date-picker; unported dep date-range-picker; unported dep multiselect; unported dep select; interaction (manual-triage tier).
...
const Autosuggest = (() => null) as any; // unported dep: autosuggest
import Button from '../../../lib/components/button/index.js';
const ButtonDropdown = (() => null) as any; // unported dep: button-dropdown
const DatePicker = (() => null) as any; // unported dep: date-picker
const DateRangePicker = (() => null) as any; // unported dep: date-range-picker
...
const Multiselect = (() => null) as any; // unported dep: multiselect
import Popover from '@components/Popover.pui';
const Select = (() => null) as any; // unported dep: select
...
describe('Modal component', () => {
	...
	describe('size property', () => {
		it('displays correct size', () => {
			(['small', 'medium', 'large', 'x-large', 'xx-large', 'max'] as any.Size[]).forEach((size) => {
	...
	describe('position property', () => {
		...
		it('displays correct position', () => {
			(['center', 'top'] as any.Position[]).forEach((position) => {
	...
	describe('Dismiss event', () => {
		it('informs about dismiss by clicking overlay', () => {
			...
			expect(onDismissSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { reason: 'overlay' } }),
			);
		});
		...
		it('informs about dismiss by pressing ESC', () => {
			...
			expect(onDismissSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { reason: 'keyboard' } }),
			);
		});
		...
		it('informs about dismiss by clicking close button', () => {
			...
			wrapper.findDismissButton().click();
			expect(onDismissSpy).toHaveBeenCalledWith(
				expect.objectContaining({ detail: { reason: 'closeButton' } }),
			);
		});
	});
	...
	describe('disable body scroll behavior', () => {
		...
		it('adjusts body padding to avoid resizing due to removed scroll bar', () => {
			...
			const hasBodyScrollbar = () =>
				0 < document.body.clientWidth && document.body.clientWidth < window.innerWidth;
			...
			if (hasBodyScrollbar()) {
				...
				console.info(
					'[skipped] Verification of document.body padding-right adjustments for scrollbar',
				);
			}
	...
	describe('contexts', () => {
		it('should not label inputs using a FormField context outside the modal', () => {
			...
			expect(wrapper.findContent().findInput()!.getElement()).not.toHaveAccessibleName(
				'Outer label',
			);
		});
	});
