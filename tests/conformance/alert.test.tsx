...
describe('Alert Component', () => {
	describe('structure', () => {
		...
		it('correct button text', () => {
			const { wrapper } = renderAlert({ buttonText: 'Button text' });
			expect(wrapper.findActionButton()!.findTextRegion()!.getElement()).toHaveTextContent(
				'Button text',
			);
		});
		...
		it('status icon can have a label', () => {
			...
			expect(wrapper.find('[role="img"]')!.getElement()).toHaveAttribute(
				'aria-label',
				'status: info',
			);
		});
...
describe('Style API', () => {
	test('custom properties', () => {
		...
		expect(
			getComputedStyle(wrapper.findRootElement().getElement()).getPropertyValue('background'),
		).toBe('rgb(255, 255, 255)');
		expect(
			getComputedStyle(wrapper.findRootElement().getElement()).getPropertyValue('border-color'),
		).toBe('rgb(0, 0, 0)');
		expect(
			getComputedStyle(wrapper.findRootElement().getElement()).getPropertyValue('border-radius'),
		).toBe('8px');
		expect(
			getComputedStyle(wrapper.findRootElement().getElement()).getPropertyValue('border-width'),
		).toBe('2px');
		expect(getComputedStyle(wrapper.findRootElement().getElement()).getPropertyValue('color')).toBe(
			'rgb(0, 0, 0)',
		);
		expect(
		...
		const dismissButton = wrapper.findDismissButton()!.getElement();
		expect(getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleColorActive)).toBe(
			'rgb(12, 136, 22)',
		);
		expect(getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleColorDefault)).toBe(
			'rgb(189, 37, 40)',
		);
		expect(getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleColorHover)).toBe(
			'rgb(119, 12, 12)',
		);
		expect(
			getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleFocusRingBorderColor),
		).toBe('rgb(23, 31, 118)');
		expect(
			getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleFocusRingBorderRadius),
		).toBe('6px');
		expect(
			getComputedStyle(dismissButton).getPropertyValue(customCssProps.styleFocusRingBorderWidth),
		).toBe('4px');

		const iconElement = wrapper.findByClassName(styles.icon)!.getElement();
		expect(getComputedStyle(iconElement).getPropertyValue(customCssProps.alertIconColor)).toBe(
			'rgb(255, 0, 0)',
		);
	});
});
