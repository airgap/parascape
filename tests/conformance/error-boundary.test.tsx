// AUTO-ADAPTED from cloudscape-design/components src/error-boundary/__tests__/
// error-boundary.test.tsx via tests/conformance/codemod.mjs.
// Mechanical rewrites only: component import → .pui, createWrapper +
// render → adapter, styles → vendored, i18n/testing → passthrough provider; stubbed unresolvable ../../../lib/components/error-boundary/interfaces; stubbed unresolvable ../../../lib/components/error-boundary/internal; interaction (manual-triage tier).
...
import ErrorBoundary from '@components/ErrorBoundary.pui';
const { BuiltInErrorBoundaryProps } = __STUB; // stub: ../../../lib/components/error-boundary/interfaces
const { BuiltInErrorBoundary } = __STUB; // stub: ../../../lib/components/error-boundary/internal
const TestI18nProvider = (({ children }: any) => children) as any;
import { createWrapper } from '@conformance/adapter';

type RenderProps = Omit<
  Partial<any> & { i18nProvider?: Record<string, Record<string, string>> } & {
    [key: `data-${string}`]: string;
  },
  'children'
>;

type BoundaryTestSpec = Omit<any, 'children'> | Omit<BuiltInErrorBoundaryProps, 'children'>;

function createBoundary(id: string, other: Partial<any>): BoundaryTestSpec {
  return {
...
describe('fallback customization with renderFallback', () => {
  ...
  const renderFallback = (definition: any.FallbackProps) => (
    <div>
...
describe('component misuse', () => {
  test('error boundary shows fallback even if the required onError callback is not provided', () => {
    const ErrorBoundaryCustom = ErrorBoundary as React.ComponentType<Omit<any, 'onError'>>;
