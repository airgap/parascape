// AUTO-ADAPTED from cloudscape-design/components src/anchor-navigation/__tests__/
// anchor-navigation.test.tsx via tests/conformance/codemod.mjs.
...
import AnchorNavigation from '@components/AnchorNavigation.pui';
import { AnchorNavigationWrapper } from '@conformance/adapter';

function renderAnchorNavigation(props: any) {
...
describe('AnchorNavigation', () => {
	...
	it('applies aria-labelledby correctly', () => {
		...
		expect(wrapper.findAnchorNavigation()!.getElement()).toHaveAttribute(
			'aria-labelledby',
			'some-id',
		);
	});
	...
	describe('nested anchors', () => {
		const getParentTagChainUntilLastOL = (element: HTMLElement): string[] => {
			...
			while (currentElement.parentElement) {
				...
				if (
					...
					!['OL', 'LI'].includes(currentElement.parentElement.tagName)
				) {
					break;
				}
			}
		...
		it('renders nested levels in a sub-list', () => {
			...
			Object.keys(expectedNestings).forEach((i) => {
				...
				const tagChain = getParentTagChainUntilLastOL(
					wrapper.findAnchorByIndex(index)!.getElement()!,
				);
