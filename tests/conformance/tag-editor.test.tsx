...
import TagEditor from '@components/TagEditor.pui';
import { createWrapper, TagEditorWrapper } from '@conformance/adapter';
import { i18nStrings, MAX_KEY_LENGTH, MAX_VALUE_LENGTH } from '@conformance/tag-editor.common';
...
function renderTagEditor(props: Partial<any> = {}): RenderResult {
  ...
  return {
    wrapper,
    onChangeSpy,
    rerender: (props: Partial<any>) =>
      rerender(<TagEditor onChange={onChangeSpy} {...defaultProps} {...props} />),
  };
}

function StatefulTestComponent(props: Partial<any>) {
...
describe('Tag Editor component', () => {
  ...
  describe('Remove button', () => {
    ...
    test('renders `ariaLabel` on remove button using `removeButtonAriaLabel` on all rows', () => {
      const removeButtonAriaLabel = (item: any.Tag) => `Remove ${item.key}`;
      const { wrapper } = renderTagEditor({
  ...
  describe('focus behavior', () => {
    const renderControlledTagEditor = (props: Partial<any>) => {
    ...
    test('should focus key input when calling focus() on ref with key error', () => {
      const TestComponent = () => {
        const ref = React.useRef<any.Ref>(null);
        return (
          <>
            <TagEditor
              ref={ref}
