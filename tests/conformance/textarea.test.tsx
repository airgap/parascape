// AUTO-ADAPTED from cloudscape-design/components src/textarea/__tests__/
// textarea.test.tsx via tests/conformance/codemod.mjs.
...
import Textarea from '@components/Textarea.pui';

import styles from '@cloudscape/textarea.styles.js';

function renderTextarea(
	props: Omit<any, 'value'> & { value?: string } & React.RefAttributes<HTMLTextAreaElement> = {},
) {
...
describe('Textarea', () => {
	...
	describe('id and controlId', () => {
		...
		test('controlId is passed to the native textarea', () => {
			...
			expect(textareaWrapper.findNativeTextarea().getElement()).toHaveAttribute(
				'id',
				'something-specific',
			);
		});
	...
	describe('native attributes', () => {
		it('adds native attributes', () => {
			const { textareaWrapper } = renderTextarea({
				nativeTextareaAttributes: { 'data-testid': 'my-test-id' },
			});
			expect(
				textareaWrapper.getElement().querySelectorAll('[data-testid="my-test-id"]'),
			).toHaveLength(1);
			expect(
				textareaWrapper.getElement().querySelectorAll('textarea[data-testid="my-test-id"]'),
			).toHaveLength(1);
		});
		it('concatenates class names', () => {
			const { textarea } = renderTextarea({
				nativeTextareaAttributes: { className: 'additional-class' },
			});
