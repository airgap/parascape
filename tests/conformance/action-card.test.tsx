// AUTO-ADAPTED from cloudscape-design/components src/action-card/__tests__/
// action-card.test.tsx via tests/conformance/codemod.mjs.
...
describe('ActionCard Component', () => {
	...
	describe('header', () => {
		test('renders header as ReactNode', () => {
			...
			expect(
				wrapper.findHeader()!.getElement().querySelector('[data-testid="custom"]'),
	...
	describe('description', () => {
		test('renders description as ReactNode', () => {
			const wrapper = renderActionCard({
				description: <span data-testid="custom">Custom Description</span>,
			});
			expect(
				wrapper.findDescription()!.getElement().querySelector('[data-testid="custom"]'),
	...
	describe('children', () => {
		test('renders children as ReactNode', () => {
			...
			expect(
				wrapper.findContent()!.getElement().querySelector('[data-testid="custom"]'),
	...
	describe('focus management', () => {
		test('can be focused through the ref API', () => {
			let actionCard: any.Ref | null = null;
	...
	describe('padding options', () => {
		test('applies no-padding class to header when disableHeaderPaddings is true', () => {
			...
			expect(wrapper.findByClassName(styles.header)!.getElement()).toHaveClass(
				styles['no-padding'],
			);
		});
		...
		test('does not apply no-padding class to header by default', () => {
			const wrapper = renderActionCard({ header: 'Header' });
			expect(wrapper.findByClassName(styles.header)!.getElement()).not.toHaveClass(
				styles['no-padding'],
			);
		});
