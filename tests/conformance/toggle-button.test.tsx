...
// render → adapter, styles → vendored, firstChild→firstElementChild; interaction (manual-triage tier).
...
describe('ToggleButton Component', () => {
	...
	test('throws a warning when pressedIconName icon is not set', () => {
		...
		expect(warnOnce).toHaveBeenCalledWith(
			'ToggleButton',
			'`pressedIconName` must be provided for `pressed` state.',
		);
	});
	...
	test('throws a warning when pressedIconSvg icon is not set', () => {
		...
		expect(warnOnce).toHaveBeenCalledWith(
			'ToggleButton',
			'`pressedIconSvg` must be provided for `pressed` state.',
		);
	});
	...
	test('throws a warning when pressedIconUrl icon is not set', () => {
		...
		expect(warnOnce).toHaveBeenCalledWith(
			'ToggleButton',
			'`pressedIconUrl` must be provided for `pressed` state.',
		);
	});
