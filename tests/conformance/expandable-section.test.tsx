...
import ExpandableSection from '@components/ExpandableSection.pui';
...
function renderExpandableSection(props: any = {}): ExpandableSectionWrapper {
	const { container } = render(<ExpandableSection {...props} />);
	return createWrapper(container).findExpandableSection()!;
}

const containerizedVariants: any.Variant[] = ['container', 'stacked'];
const variantsWithActions: any.Variant[] = ['container', 'stacked', 'default', 'inline'];

describe('Expandable Section', () => {
	const variantsWithDescription: any.Variant[] = [
		...containerizedVariants,
		'default',
		'footer',
		'inline',
	];
	const variantsWithoutDescription: any.Variant[] = ['navigation'];
	const nonContainerVariants: any.Variant[] = ['default', 'footer', 'navigation', 'inline'];
	...
	describe('slots', () => {
		...
		test.each<any.Variant>(['default', 'footer', 'container', 'navigation', 'stacked', 'inline'])(
	...
	describe('dev warnings', () => {
		...
		describe('logs warning for non supported configurations', () => {
			...
			describe('other properties', () => {
				const testWarnings = (props: any) => {
					render(<ExpandableSection {...props} />);
					expect(warnOnce).toHaveBeenCalledTimes(1);
					expect(warnOnce).toHaveBeenCalledWith(
						componentName,
						expect.stringMatching(/only supported for the/),
					);
				};
...
describe('headingTagOverride', () => {
	...
	for (const variant of ['default', 'footer']) {
		describe.each<any.Variant>(['default', 'footer'])(`variant: ${variant}`, (variant) => {
	...
	for (const variant of containerizedVariants) {
		describe(`with ${variant} variant`, () => {
			...
			test('button should be under heading', () => {
				...
				expect(wrapper.find('h2')!.find('[role="button"]')!.getElement()).toHaveTextContent(
					'Header component',
				);
			});
		});
