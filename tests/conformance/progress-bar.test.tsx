...
const standaloneAndKeyvalueVariants: Array<any.Variant> = ['standalone', 'key-value'];
const allVariants: Array<any.Variant> = [...standaloneAndKeyvalueVariants, 'flash'];
const statuses: Array<any.Status> = ['success', 'error'];

const renderProgressBar = (progressBarProps: any): ProgressBarWrapper => {
	...
	describe(`Progress bar component ${variant} variant`, () => {
		describe('Progress value', () => {
			...
			test('sets aria-hidden to percentage container', () => {
				...
				expect(
					wrapper.findByClassName(styles['percentage-container'])!.getElement(),
		...
		describe('ARIA labels', () => {
			...
			describe('aria-describedby', () => {
				...
				test('associate progress element with description, additionalInfo and ariaDescribedby props', () => {
					...
					const descriptionId = screen.queryByText('dummy description')!.id;
					const additionalInfoId = screen.queryByText('dummy additional info')!.id;
					const ariaDescribedByValue = wrapper
						.find('progress')!
						.getElement()
	...
	statuses.forEach((status) => {
		describe(`Progress bar component ${variant} variant - Result state`, () => {
			...
			test('displays result button when resultButtonText is set', () => {
				...
				expect(wrapper.findResultButton()!.findTextRegion()!.getElement()).toHaveTextContent(
					'Retry',
				);
			});
