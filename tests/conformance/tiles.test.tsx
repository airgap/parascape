...
const defaultItems: any.TilesDefinition[] = [
...
describe('ref', () => {
	test('when unselected, focuses the first tile when .focus() is called', () => {
		let tilesRef: any.Ref | null = null;
		const { wrapper } = renderTiles(
			<Tiles value={null} items={defaultItems} ref={(ref) => (tilesRef = ref)} />,
		);
		tilesRef!.focus();
	...
	test('when selected, focuses the checked tile', () => {
		let tilesRef: any.Ref | null = null;
		const { wrapper } = renderTiles(
			<Tiles value="val2" items={defaultItems} ref={(ref) => (tilesRef = ref)} />,
		);
		tilesRef!.focus();
...
describe('items', () => {
	...
	test('does not trigger change handler if readOnly', () => {
		const onChange = jest.fn();
		const { wrapper } = renderTiles(
			<Tiles value={null} items={defaultItems} onChange={onChange} readOnly={true} />,
		);
	...
	test('displays the proper label', () => {
		const { wrapper } = renderTiles(
			<Tiles value={null} items={[{ value: '1', label: 'Please select' }]} />,
		);
	...
	test('displays the description', () => {
		const { wrapper } = renderTiles(
			<Tiles
				value={null}
				items={[{ value: '1', label: 'Please select', description: 'Tile description test' }]}
			/>,
		);
		expect(wrapper.findItemByValue('1')!.findDescription()!.getElement()).toHaveTextContent(
			'Tile description test',
		);
	});
...
describe('correct number of columns', () => {
	...
	for (let nItems = 1; nItems <= 10; nItems++) {
		it(`can have ${nItems} columns`, () => {
			...
			expect(internalWrapper.findColumns().getElement()).toHaveClass(
				styles[`column-${expected[nItems]}`],
			);
...
describe('value', () => {
	...
	test('fires change event when a tile is selected', () => {
		const onChange = jest.fn();
		const { wrapper } = renderTiles(
			<Tiles value={null} items={defaultItems} onChange={onChange} />,
		);
	...
	describe('tile controlId', () => {
		function check(tile: TileWrapper, controlId: string) {
			...
			expect(tile.findNativeInput().getElement()).toHaveAttribute(
				'aria-labelledby',
				`${controlId}-label`,
			);
		}
	...
	describe('aria-required', () => {
		test(`is set for value=true`, () => {
			const { wrapper } = renderTiles(
				<Tiles ariaRequired={true} value={null} items={defaultItems} />,
			);
		...
		test(`is not set for value=true`, () => {
			const { wrapper } = renderTiles(
				<Tiles ariaRequired={false} value={null} items={defaultItems} />,
			);
