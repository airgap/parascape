...
function renderBox(props: any = {}): BoxWrapper {
...
function testClassNamesForProperty<PropertyType>(
	propertyName: string,
...
describe('Box', () => {
		...
		describe(`${spacingType} property`, () => {
			...
			test(`sets the right classNames for side-specific ${spacingType}s`, () => {
				...
				sides.forEach((side) => {
					sizes.forEach((size) => {
						...
						expect(boxWrapper.getElement()).toHaveClass(
							styles[`${spacingType.slice(0, 1)}-${side}-${size}`],
						);
					});
				});
			});
			test(`sets the right classNames for ${spacingType}s when multiple sides are provided`, () => {
				...
				const expectedClassNames = [
					...
					'horizontal-m',
					'vertical-l',
				].map((suffix) => styles[`${spacingType.slice(0, 1)}-${suffix}`]);
	...
	testClassNamesForProperty<any.Display>(
		'display',
		['block', 'inline', 'inline-block', 'none'],
		'd',
	);
	testClassNamesForProperty<any.TextAlign>('textAlign', ['left', 'center', 'right'], 't');
	testClassNamesForProperty<any.Float>('float', ['left', 'right'], 'f');
	testClassNamesForProperty<any.FontSize>(
		'fontSize',
		[
			'body-s',
			'body-m',
	...
	testClassNamesForProperty<any.FontWeight>(
		'fontWeight',
		['light', 'normal', 'bold'],
		'font-weight',
	);
	testClassNamesForProperty<any.Color>(
		'color',
		[
			'inherit',
			'text-label',
