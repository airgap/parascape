...
// render → adapter, styles → vendored, jest.mock → hoisted vi.mock; stubbed unresolvable ../../../lib/components/live-region/controller.js; stubbed unresolvable ../../internal/analytics/__tests__/mocks; stubbed unresolvable ../constant; interaction (manual-triage tier).
...
import { act, render as reactRender, waitFor } from '@conformance/adapter';
...
const { mockInnerText } = __STUB; // stub: ../../internal/analytics/__tests__/mocks
const { TIMEOUT_FOR_ENTERING_ANIMATION } = __STUB; // stub: ../constant
import { createFlashbarWrapper, findList, testFlashDismissal } from '@conformance/flashbar.common';
...
vi.mock('../../../lib/components/internal/hooks/use-visual-mode', async (importOriginal) => {
  const originalVisualModeModule = await vi.importActual('../../../lib/components/internal/hooks/use-visual-mode');
  return {
...
describe('Flashbar component', () => {
  describe.each([true, false])('withAnimations=%s', withAnimations => {
    ...
    test.each([['success'], ['error'], ['info'], ['warning'], ['in-progress']] as any.Type[][])(
