// AUTO-ADAPTED from cloudscape-design/components src/input/__tests__/
// input.test.tsx via tests/conformance/codemod.mjs.
...
function renderInput(
	props: Omit<any, 'value'> & { value?: string } & React.RefAttributes<any.Ref> = {},
) {
...
describe('Input', () => {
	...
	describe('type', () => {
		...
		test('can be set to of the valid types', () => {
			const types: any.Type[] = ['text', 'password', 'search', 'number', 'email', 'url'];
	...
	describe('native attributes', () => {
		it('adds native attributes', () => {
			const { container } = render(
				<Input value="" nativeInputAttributes={{ 'data-testid': 'my-test-id' }} />,
			);
			expect(container.querySelectorAll('[data-testid="my-test-id"]')).toHaveLength(1);
		});
		it('concatenates class names', () => {
			const { container } = render(
				<Input value="" nativeInputAttributes={{ className: 'additional-class' }} />,
			);
