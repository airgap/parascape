...
describe('Slots', () => {
	...
	it('renders the popover header correctly with portal', () => {
		...
		wrapper.findTrigger().click();
		expect(wrapper.findHeader({ renderWithPortal: true })!.getElement()).toHaveTextContent(
			'Memory error',
		);
	});
	...
	it('renders the popover content correctly with portal', () => {
		...
		wrapper.findTrigger().click();
		expect(wrapper.findContent({ renderWithPortal: true })!.getElement()).toHaveTextContent(
			'There was an error.',
		);
	});
});
...
describe('Focus behavior', () => {
	...
	it('moves focus to the dismiss button on open if dismiss button is present - with portal', () => {
		...
		expect(document.activeElement).toBe(
			wrapper.findDismissButton({ renderWithPortal: true })!.getElement(),
		);
	});
	...
	it('does not move focus on open if dismiss button is not present - with portal', () => {
		...
		expect(document.activeElement).not.toBe(
			wrapper.findBody({ renderWithPortal: true })!.getElement(),
		);
	});
});
...
describe('i18n', () => {
	it('uses dismissAriaLabel from i18n provider', () => {
		...
		wrapper.findTrigger().click();
		expect(wrapper.findDismissButton()!.getElement()).toHaveAttribute(
			'aria-label',
			'Custom dismiss',
		);
	});
