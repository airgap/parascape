...
// render → adapter, styles → vendored, stripped a11y/mocks helper; interaction (manual-triage tier).
...
describe('Button Component', () => {
	...
	describe('i18n', () => {
		test('supports providing externalIconAriaLabel from i18n provider', () => {
			const { container } = render(
				<TestI18nProvider
					messages={{ button: { 'i18nStrings.externalIconAriaLabel': 'External test' } }}
				>
	...
	describe.each(['primary', 'normal', 'icon'] as const)(
	...
	describe('formAction property', () => {
		function renderButtonInForm(props: any = {}) {
	...
	describe('iconAlign property', () => {
		...
		(['icon', 'inline-icon'] as any.Variant[]).forEach((variant) => {
	...
	describe('URL sanitization', () => {
		...
		test('does not throw an error when a safe javascript: URL is passed', () => {
			...
			expect((element.getElement() as unknown as HTMLAnchorElement).href).toBe(
				'javascript:void(0)',
			);
...
describe('Style API', () => {
	test('custom properties', () => {
		...
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBackgroundActive),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
				customCssProps.styleBackgroundDefault,
			),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
				customCssProps.styleBackgroundDisabled,
			),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBackgroundHover),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
		...
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
				customCssProps.styleBorderColorDisabled,
			),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBorderColorHover),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleColorActive),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleColorDefault),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleColorDisabled),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleColorHover),
		).toBe('#fff');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
		...
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBoxShadowActive),
		).toBe('0 0 10px red');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBoxShadowDefault),
		).toBe('0 0 5px blue');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(
				customCssProps.styleBoxShadowDisabled,
			),
		).toBe('0 0 2px gray');
		expect(
			getComputedStyle(wrapper.getElement()).getPropertyValue(customCssProps.styleBoxShadowHover),
		).toBe('0 0 8px green');
	});
});
