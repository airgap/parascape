...
test('can receive focus if readOnly', () => {
	...
	const { wrapper } = renderToggle(
		<Toggle ref={(ref) => (toggleRef = ref)} checked={false} readOnly={true} />,
	);
...
test('can be focused via API', () => {
	...
	const { wrapper } = renderToggle(
		<Toggle ref={(ref) => (toggleRef = ref)} checked={false} onFocus={onFocus} />,
	);
	expect(toggleRef).toBeDefined();
...
test('Should set aria-describedby and aria-labelledby from Formfield', () => {
	...
	const toggleInputAriaDescribedby = toggleWrapper
		.findNativeInput()
		.getElement()
		.getAttribute('aria-describedby');
	const toggleInputAriaLabelledby = toggleWrapper
		.findNativeInput()
		.getElement()
...
test('all style api properties', () => {
	...
	expect(
		getComputedStyle(toggleControl).getPropertyValue(customCssProps.styleFocusRingBorderColor),
	).toBe('magenta');
	expect(
		getComputedStyle(toggleControl).getPropertyValue(customCssProps.styleFocusRingBorderRadius),
	).toBe('10px');
	expect(
		getComputedStyle(toggleControl).getPropertyValue(customCssProps.styleFocusRingBorderWidth),
	).toBe('5px');
...
describe('native attributes', () => {
	it('adds native attributes', () => {
		const { container } = render(
			<Toggle checked={true} nativeInputAttributes={{ 'data-testid': 'my-test-id' }} />,
		);
	...
	it('concatenates class names', () => {
		const { container } = render(
			<Toggle checked={true} nativeInputAttributes={{ className: 'additional-class' }} />,
		);
