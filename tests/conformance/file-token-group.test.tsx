// AUTO-ADAPTED from cloudscape-design/components src/file-token-group/__tests__/
// file-token-group.test.tsx via tests/conformance/codemod.mjs.
// Mechanical rewrites only: component import → .pui, createWrapper +
// render → adapter, styles → vendored, i18n/testing → passthrough provider; jest.mock → hoisted vi.mock; firstChild→firstElementChild; interaction (manual-triage tier).
...
import FileTokenGroup from '@components/FileTokenGroup.pui';
...
import testStyles from '@cloudscape/file-token-group.test-classes.js';
...
function StatefulFileTokenGroup({ items: initialItems = [], ...rest }: Partial<any>) {
...
describe('File loading', () => {
  ...
  test('Spinner added when loading', () => {
    ...
    expect(wrapper.findFileToken(1)?.getElement().firstElementChild).toHaveClass(styles.loading);
    expect(wrapper.findFileToken(2)?.getElement().firstElementChild).not.toHaveClass(styles.loading);
  });
});
