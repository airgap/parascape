// AUTO-ADAPTED from cloudscape-design/components src/time-input/__tests__/
// time-input.test.tsx via tests/conformance/codemod.mjs.
...
import TimeInput from '@components/TimeInput.pui';

const defaultProps: any = { value: '' };
...
function renderTimeInput(props = defaultProps, autoFocus = true) {
	...
	const { container } = render(
		<TimeInput onChange={onChangeSpy} onBlur={onBlurSpy} onFocus={onFocusSpy} {...props} />,
	);
	...
	if (autoFocus) {
		...
		wrapper
			.findNativeInput()
			.getElement()
			.setSelectionRange(props.value.length, props.value.length);
	}
