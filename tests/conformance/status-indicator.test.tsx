...
describe('StatusIndicator', () => {
	test('Renders the class corresponding to the type', function () {
		const { container } = render(
			<StatusIndicator type="error">The creation failed</StatusIndicator>,
		);
	...
	it('Renders the icon if the type is not "loading"', function () {
		const { container } = render(
			<StatusIndicator type="stopped">The instance stopped</StatusIndicator>,
		);
	...
	it('Renders the spinner for type "loading"', function () {
		const { container } = render(
			<StatusIndicator type="loading">The resource is loading</StatusIndicator>,
		);
	...
	it('Sets aria-label to the value of the iconAriaLabel prop', function () {
		...
		const statusIndicatorWrapper = createWrapper(container.parentElement!).findStatusIndicator()!;
		expect(statusIndicatorWrapper.findByClassName(styles.icon)!.getElement()).toHaveAttribute(
			'aria-label',
			ariaLabel,
		);
	});
	...
	describe('native attributes', () => {
		it('adds native attributes', () => {
			const { container } = render(
				<StatusIndicator type="info" nativeAttributes={{ 'data-testid': 'my-test-id' }} />,
			);
