...
const defaultItems: any.RadioButtonDefinition[] = [
...
test('hides the decorative icon from assistive technology', function () {
	const { wrapper } = renderRadioGroup(
		<RadioGroup name="test" value={null} items={defaultItems} />,
	);
...
describe('name', () => {
	...
	test('generates a random name if not provided', () => {
		const { wrapper, rerender } = renderRadioGroup(
			<RadioGroup value="val1" name="test" items={defaultItems} />,
		);
...
describe('items', () => {
	...
	test('can be disabled', () => {
		...
		const { wrapper, rerender } = renderRadioGroup(
			<RadioGroup
				value={null}
				items={[defaultItems[0], { ...defaultItems[1], disabled: true }]}
				onChange={onChange}
			/>,
		);
	...
	test('Adds aria-readonly when readOnly is set to true', () => {
		const { wrapper } = renderRadioGroup(
			<RadioGroup value="val1" name="test" items={defaultItems} readOnly={true} />,
		);
	...
	test('can receive focus if readOnly', () => {
		let radioGroupRef: any.Ref | null = null;
		const { wrapper } = renderRadioGroup(
			<RadioGroup
				value={null}
				items={defaultItems}
				ref={(ref) => (radioGroupRef = ref)}
				readOnly={true}
			/>,
		);
		radioGroupRef!.focus();
	...
	test('displays the proper label', () => {
		const { wrapper } = renderRadioGroup(
			<RadioGroup value={null} items={[{ value: '1', label: 'Please select' }]} />,
		);
	...
	test('displays no label text when label is empty', () => {
		const { wrapper } = renderRadioGroup(
			<RadioGroup value={null} items={[{ value: '1', label: '' }]} />,
		);
	...
	test('displays the description', () => {
		...
		expect(wrapper.findButtons()[0].findDescription()!.getElement()).toHaveTextContent(
...
describe('value', () => {
	test('selects the right button when value changes', () => {
		const { wrapper, rerender } = renderRadioGroup(
			<RadioGroup value={null} items={defaultItems} />,
		);
	...
	test('fires change event when a button is selected', () => {
		const onChange = jest.fn();
		const { wrapper } = renderRadioGroup(
			<RadioGroup value={null} items={defaultItems} onChange={onChange} />,
		);
	...
	test('updates value via onChange handler (controlled component)', () => {
		function ComponentWrapper() {
			const [value, setValue] = useState<any['value']>(null);
			return (
				<RadioGroup
					value={value}
					items={defaultItems}
	...
	test('deselects all buttons when reset to an empty value', () => {
		const { wrapper, rerender } = renderRadioGroup(
			<RadioGroup value="val1" items={defaultItems} />,
		);
		const input1 = wrapper.findInputByValue('val1')!;
		const input2 = wrapper.findInputByValue('val2')!;
	...
	test('Keeps selected item when rerendering', () => {
		const { wrapper, rerender } = renderRadioGroup(
			<RadioGroup value="val2" items={defaultItems} />,
		);
		...
		rerender(
			<RadioGroup
				value="val2"
				items={[defaultItems[0], { ...defaultItems[1], label: 'New label...' }]}
			/>,
		);
	...
	describe('radiobutton controlId', () => {
		function check(radioButton: RadioButtonWrapper, controlId: string) {
			...
			expect(radioButton.findNativeInput().getElement()).toHaveAttribute(
				'aria-labelledby',
				`${controlId}-label`,
			);
		}
	...
	describe('aria-required', () => {
		test(`is set for value=true`, () => {
			const { wrapper } = renderRadioGroup(
				<RadioGroup ariaRequired={true} value={null} items={defaultItems} />,
			);
		...
		test(`is not set for value=true`, () => {
			const { wrapper } = renderRadioGroup(
				<RadioGroup ariaRequired={false} value={null} items={defaultItems} />,
			);
...
test('all style api properties', function () {
	...
	const outerCircle = wrapper
		.findByClassName(radioButtonStyles['styled-circle-border'])!
		.getElement();
	const innerCircle = wrapper
		.findByClassName(radioButtonStyles['styled-circle-fill'])!
		.getElement();
	...
	expect(getComputedStyle(control).getPropertyValue(customCssProps.styleFocusRingBorderColor)).toBe(
		'orange',
	);
	expect(
		getComputedStyle(control).getPropertyValue(customCssProps.styleFocusRingBorderRadius),
	).toBe('2px');
	expect(getComputedStyle(control).getPropertyValue(customCssProps.styleFocusRingBorderWidth)).toBe(
		'1px',
	);
});
