// AUTO-ADAPTED from cloudscape-design/components src/date-picker/__tests__/
// date-picker.test.tsx via tests/conformance/codemod.mjs.
// Mechanical rewrites only: component import → .pui, createWrapper +
// render → adapter, styles → vendored, i18n/testing → passthrough provider; stubbed unresolvable ../../../lib/components/internal/events; interaction (manual-triage tier).
...
import { DatePickerWrapper } from '@conformance/adapter';
...
function renderDatePicker(props: any = defaultProps) {
...
describe('Date picker - direct date input', () => {
  ...
  test('propagates input format to date input', () => {
    ...
    const props: any = { ...defaultProps, format: 'long-localized', locale: 'en-GB', onChange };
  ...
  test('does not allow changing autocomplete values', () => {
    const { wrapper } = renderDatePicker({ ...defaultProps, autocomplete: true } as any);
    expect(wrapper.findNativeInput().getElement()).toHaveAttribute('autocomplete', 'off');
  });
  ...
  describe('blur event', () => {
    let onBlurSpy: jest.Mock<NonCancelableEventHandler<any.ChangeDetail>>,
  ...
  describe('change event', () => {
    let onChangeSpy: jest.Mock<NonCancelableEventHandler<any.ChangeDetail>>, wrapper: DatePickerWrapper;
