// AUTO-ADAPTED from cloudscape-design/components src/text-filter/__tests__/
// text-filter.test.tsx via tests/conformance/codemod.mjs.
...
test('should apply filteringPlaceholder', () => {
	const { wrapper } = renderTextFilter(
		<TextFilter filteringText="" filteringPlaceholder="search placeholder" />,
	);
	expect(wrapper.findInput().findNativeInput().getElement()).toHaveAttribute(
		'placeholder',
		'search placeholder',
	);
});
...
test('should focus input via function', () => {
	...
	const { wrapper } = renderTextFilter(
		<TextFilter
			ref={(value) => (ref = value!)}
			filteringText=""
			filteringPlaceholder="search placeholder"
		/>,
	);
	ref!.focus();
...
test('should fire filtering change when the text input changes', () => {
	...
	expect(onChange).toHaveBeenCalledWith(
		expect.objectContaining({ detail: { filteringText: 'filtering' } }),
	);
});
...
test('should fire onDelayedChange with a delay', async () => {
	...
	const { wrapper } = renderTextFilter(
		<TextFilter filteringText="" onDelayedChange={onDelayedChange} />,
	);
...
test('should pass the label to the filtering input if defined', () => {
	const { wrapper } = renderTextFilter(
		<TextFilter filteringText="" filteringAriaLabel="it is a filter" />,
	);
	expect(wrapper.findInput().findNativeInput().getElement()).toHaveAttribute(
		'aria-label',
		'it is a filter',
...
test('should pass through properties using form field context', () => {
	...
	expect(document.getElementById(nativeInput.getAttribute('aria-labelledby')!)).toHaveTextContent(
		'Test label',
	);
	expect(document.getElementById(nativeInput.getAttribute('aria-describedby')!)).toHaveTextContent(
		'Test description',
	);
...
describe('countText', () => {
	...
	test('displays the text when all conditions met', () => {
		const { wrapper } = renderTextFilter(
			<TextFilter filteringText="test" countText="10 matches" />,
		);
	...
	describe('live announcement', () => {
		...
		test('re-announce the live announcement when countText changes', () => {
			...
			const { rerender } = renderTextFilter(
				<TextFilter filteringText={filteringText} countText="10 matches" />,
			);
			jest.runAllTimers();
			expect(getPoliteRegion()).toHaveTextContent('10 matches');
...
describe('disableBrowserAutocorrect', () => {
	...
	test('does not modify autocorrect features when falsy', () => {
		const { wrapper } = renderTextFilter(
			<TextFilter filteringText="" disableBrowserAutocorrect={false} />,
		);
		const input = wrapper.findInput().findNativeInput().getElement();
	...
	test('can disabled autocorrect features when set', () => {
		const { wrapper } = renderTextFilter(
			<TextFilter filteringText="" disableBrowserAutocorrect={true} />,
		);
		const input = wrapper.findInput().findNativeInput().getElement();
...
test('check a11y', async () => {
	const { wrapper } = renderTextFilter(
		<TextFilter filteringText="" filteringAriaLabel="filter instances" />,
	);
