...
// render → adapter, styles → vendored, jest.mock → hoisted vi.mock; stubbed unresolvable ../../../lib/components/internal/keycode; interaction (manual-triage tier).
...
import { act, fireEvent, render } from '@conformance/adapter'; // unsupported: screen

import { warnOnce } from '@cloudscape-design/component-toolkit/internal';

import ButtonDropdown from '@components/ButtonDropdown.pui';
import InternalButtonDropdown from '@components/ButtonDropdown.pui';
...
[false, true].forEach(expandToViewport => {
  describe(`ButtonDropdown Component ${expandToViewport ? 'with portal' : 'without portal'}`, () => {
    (['normal', 'primary', 'icon'] as Array<any['variant']>).forEach(variant => {
      describe(`"${variant}" variant`, () => {
        const props = { expandToViewport, variant };

        test('can be focused through the API', () => {
          let buttonDropdown: any.Ref | null = null;
